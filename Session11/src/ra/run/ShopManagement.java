...
public class ShopManagement {
    ...
    public static void updateCatalog() {
        //1. Nhập vào mã danh mục cần cập nhập
        System.out.println("Nhập vào mã danh mục cần cập nhập");
        int catalogUpdateId = Integer.parseInt(scanner.nextLine());
        //2. Tìm index danh mục cần cập nhập trong danh sách
        int indexUpdate = getIndexCatalogById(catalogUpdateId);
        if (indexUpdate >= 0) {
            categoriesList.remove(indexUpdate);
            // Nhập dữ liệu cho danh mục cần cập nhập
            Categories catalogUpdate = new Categories();
            catalogUpdate.inputData(scanner, categoriesList, productList);
            // Add dữ liệu vào mảng
            categoriesList.add(indexUpdate, catalogUpdate);
        } else {
            System.err.println("Mã danh mục cần cập nhập không tồn tại");
        }
    }
    ...
    public static void deleteCatalog() {
        ...
        int catalogDeleteId = Integer.parseInt(scanner.nextLine());
        // Tìm index của danh mục cần xóa trong danh sách
        int indexDelete = getIndexCatalogById(catalogDeleteId);
        if (indexDelete >= 0) {
            //Kiểm tra trong danh mục có sản phẩm chưa
            boolean isExist = false;
            for (Product product : productList
            ) {
                if (product.getCatalogId() == catalogDeleteId) {
                    isExist = true;
                    break;
                }
            }
            if (!isExist) {
                ...
                System.out.println("Đã xóa xong danh mục");
            } else {
                System.err.println("Danh mục đã có sản phẩm, không thể xóa");
            }
        } else {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    ShopManagement.updateProductPriceByProductId();
                    break;
                case 4:
    ...
    public static void updateProductPriceByProductId() {
        System.out.println("Nhập vào mã sản phẩm cần cập nhập");
        String updateProductId;
        boolean isExit = true;
        do {
            updateProductId = scanner.nextLine();
            if (validateInputProductId(updateProductId)) {
                isExit = false;
            }
        } while (isExit);
        //Tìm index sản phẩm cần cập nhập trong danh sách
        int indexUpdate = getIndexProductById(updateProductId);
        if (indexUpdate >= 0) {
            // Nhập vào giá cần cập nhập
            System.out.println("Nhập vào giá cập nhập");
            float priceUpdate = Float.parseFloat(scanner.nextLine());
            // Gán priceUpdate vào đối tượng cần cập nhập giá
            productList.get(indexUpdate).setPrice(priceUpdate);
            System.out.println("Đã cập nhập thành công");
        } else {
            System.err.println("Mã sản phẩm cần cập nhập không tồn tại");
        }
    }
    ...
    public static void deleteProductByProductId() {
        ...
        do {
            ...
            if (validateInputProductId(deleteProductId)) {
                isExit = false;
            }
        } while (isExit);
        // Tìm index của sản phẩm cần xóa trong danh sách
        int indexDelete = getIndexProductById(deleteProductId);
        if (indexDelete >= 0) {
            // Thực hiện xóa sản phẩm
            productList.remove(indexDelete);
        } else {
            System.err.println("Mã sản phẩm cần xóa không tồn tại");
        }
    }
    ...
    public static boolean validateInputProductId(String productId) {
        if (productId.length() == 5) {
            ...
            System.err.println("Vui lòng nhập mã sản phẩm cập nhập gồm 5 ký tự");
            return false;
        }
    }
